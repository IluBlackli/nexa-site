...
(function () {
  ...
  var I18N = {
    pt: {
      ...
      'work.3.desc': 'Site para uma marca de açaí — história da marca, menu de produtos, galeria e contacto para encomendas.',
      'work.verSite': 'Ver site →', 'work.verProjeto': 'Ver projeto →',
      'cta.title': 'Contacta-nos e cria um espaço personalizado para a tua empresa.',
      'cta.text': 'Do primeiro esboço ao lançamento — um site, uma plataforma ou um sistema com IA feito à medida do teu negócio.',
    ...
    es: {
      ...
      'work.3.desc': 'Web para una marca de açaí — historia de la marca, menú de productos, galería y contacto para pedidos.',
      'work.verSite': 'Ver sitio →', 'work.verProjeto': 'Ver proyecto →',
      'cta.title': 'Contáctanos y crea un espacio personalizado para tu empresa.',
      'cta.text': 'Del primer boceto al lanzamiento — una web, una plataforma o un sistema con IA hecho a la medida de tu negocio.',
    ...
    en: {
      ...
      'work.3.desc': 'Website for an açaí brand — brand story, product menu, gallery and contact for orders.',
      'work.verSite': 'View site →', 'work.verProjeto': 'View project →',
      'cta.title': 'Get in touch and build a custom space for your company.',
      'cta.text': 'From the first sketch to launch — a website, a platform or an AI-powered system, tailored to your business.',
  ...
  if (searchForms.length) {
    ...
    var buildIndex = function () {
      var idx = [];
      var add = function (title, extra, action) {
        idx.push({ title: title, hay: norm(title + ' ' + (extra || '')), snippet: (extra || '').replace(/\s+/g, ' ').trim(), action: action });
      };
      $$('main > section[id]').forEach(function (sec) {
        var label = sec.querySelector('.block__label');
        var sub = sec.querySelector('.block__title');
        if (label) add(label.textContent.trim(), sub ? sub.textContent.trim() : '', function () { smoothTo('#' + sec.id); });
      });
      $$('#servicos .card, #trabalhos .card').forEach(function (c) {
        var t = c.querySelector('.card__title');
        var d = c.querySelector('.card__desc');
        var sec = c.closest('section[id]');
        if (t) add(t.textContent.trim(), d ? d.textContent.trim() : '', function () { smoothTo('#' + sec.id); });
      });
      $$('#faq .faq__item').forEach(function (it) {
        var q = it.querySelector('.faq__q span[data-i18n]') || it.querySelector('.faq__q');
        var a = it.querySelector('.faq__a');
        if (q) add(q.textContent.trim(), a ? a.textContent.trim() : '', function () {
          smoothTo('#faq');
          it.open = true;
        });
      });
      $$('#sobre .about__block').forEach(function (b) {
        var h = b.querySelector('h3');
        add(h ? h.textContent.trim() : 'Sobre', b.textContent.trim(), function () { smoothTo('#sobre'); });
      });
      var contactLbl = ($('[data-i18n="nav.contacto"]') || {}).textContent || 'Contacto';
      add(contactLbl, 'email whatsapp telefone reuniao reunion meeting calendly nexxacreative.web@gmail.com 965 262 788', function () {
        var t = $('[data-contact]');
        if (t) t.click();
      });
